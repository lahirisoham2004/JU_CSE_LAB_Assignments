...
def save_file(sender, recipient, filename, data):
    conn = sqlite3.connect("chat.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO files (sender, recipient, filename, data) VALUES (?, ?, ?, ?)", (sender, recipient, filename, data))
    conn.commit()
    conn.close()
...
def handle_file_transfer(client, username):
    ...
    try:
        # Receive file information: recipient:filename:filesize
        file_info = cipher.decrypt(client.recv(1024)).decode().split(":", 2) # Split recipient and filename
        if len(file_info) != 3:
            print(f"Invalid file info received from {username}")
            return

        recipient, filename, filesize_str = file_info
        filesize = int(filesize_str)

        if filesize > MAX_FILE_SIZE:
             client.send(cipher.encrypt("FILE_TOO_LARGE".encode()))
             return


        # Receive file data
        file_data = client.recv(filesize)

        if len(file_data) != filesize:
            print(f"Incomplete file received from {username}")
            return

        save_file(username, recipient, filename, file_data)  # Save the file to the database
        broadcast(cipher.encrypt(f"{username} sent a file '{filename}' to {recipient}".encode()), recipient=recipient) # Notify recipient
    ...
    try:
        ...
        if username:
            ...
            while True:
                ...
                if message.startswith("FILE:"):  # Check if the message is a file transfer request
                    threading.Thread(target=handle_file_transfer, args=(client, username), daemon=True).start()
