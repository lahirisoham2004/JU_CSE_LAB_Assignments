...
void MainWindow::draw_bresenham_line(int x1, int y1, int x2, int y2)
{
    ...
    colorPoint(xGrid, yGrid, 255, 255, 0, gridOffset);  // Color the initial point
    QVector<QPoint> pts;
...
void MainWindow::on_DrawPoly_clicked()
{
    ...
    for(int i = 0; i<sides-1; i++){
        ...
        draw_dda_line(coord1.x(), coord1.y(), coord2.x(), coord2.y());
    }
...
void MainWindow::boundaryFill(int x, int y){
    ...
    qDebug()<<clr<<QPoint(x,y);
    //QColor clr1(0, 255, 0);
    QColor cl(qRgb(0, 255, 0));
    if(clr == cl || x>width || y>height || x<0 || y<0){
        return;
    }
    //qDebug()<<x<<y<<'\n';
    colorPoint(x, y, ui->r->value(), ui->g->value(), ui->b->value(), gridOffset);
