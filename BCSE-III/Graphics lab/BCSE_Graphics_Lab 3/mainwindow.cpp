...
void MainWindow::on_axisWidth_valueChanged(int arg1)
{
    ...
    int centerX = width / 2;
    int centerY = height / 2;
    qDebug()<<width<<height<<centerX<<centerY;
    // Draw horizontal axis
    for (int x = 0; x < width; ++x) {
        colorPoint(x, centerY, 255, 0, 0, arg1); // Black color
    }

    // Draw vertical axis
    for (int y = 0; y < height; ++y) {
        colorPoint(centerX, y, 255, 0, 0, arg1); // Black color
    }
...
void MainWindow::on_pushButton_2_clicked()//Bressenham Line Drawing Algorithm
{
    if (clickedPoints.size() < 2) {
        // Not enough points to draw a line
        return;
